 ...
 * on removable media. You'll need to mount the phone on a computer to retrieve
 * the files, or have a FileManager app installed on the phone. 
 */
public class FileWriter {
	private String TAG = "SpeedData";
	...
	private FileWriter() {
		...
		// NO COLONS!! VERBOTEN in AndroidFileNames
	...
	private Boolean writeToSyslogFile() {
		if (syslogFile != null) {
			...
			Log.w(TAG, "can't write syslog msg");
		}
		return true;
	}
	...
	private Boolean writeToDataFile() {
		if (dataFile != null) {
			...
			Log.w(TAG, "can't write datafile");
		}
		return true;
	}
